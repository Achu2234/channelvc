    ...
    if audio:
        ...
        if duration > DURATION_LIMIT:
            raise DurationLimitError(
                f'The Provided Video Is {duration} Minutes!',
            )
        ...
        return await message.reply_text('Provide Me YouTube Link To Play!')
    result = await play(message.chat.id, file_path)
    if result != -1:
        await message.reply_text(f'Queued at Position {result}!')
    else:
        await message.reply_text('Playing Song at your channel vc!')
